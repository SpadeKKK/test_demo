...
def fe_cal(dataname):
    suffix = f'{dataname}' # change the folder suffix   
    mzml_path = f"data_{suffix}/mzml_data_conversion_SRMspectra/" # folder path to mzml mass spec files
    data_info_file = f"data_{suffix}/data_{suffix}.csv" # change the filename
    
    
    data_info = pd.read_csv(data_info_file) # \data_MLIT_T
    data_info_selected = data_info[(data_info['Current criteria']=='TB') | (data_info['Current criteria']=='Not TB')] 
    
    label_list = data_info_selected['Current criteria'].values
    ...
    data_info_selected_sample_id = data_info_selected['MaskedID'].values
    sample_id_list = [x for x in data_info_selected_sample_id]
    ...
    os.makedirs(f"data_{suffix}", exist_ok=True)
    datafile_name = f"saved_data_{suffix}.pkl"
    with open(f"data_{suffix}/{datafile_name}", "wb") as f:
        pickle.dump(data, f)
