...
def oritrain(X_trainsets, y_trainsets, dtname, fc):
    ...
    def setup_scores(i):
        ...
        if i not in y_prob_final:
            y_prob_final[i] = np.ones(len(y_trainsets))*-1
    ...
    fold = 0
    for train_index, test_index in shuffle_split.split(X=X_trainsets, y=y_trainsets):
        fold += 1
        print(f"Running fold {fold} of {n_repeats}...")
        X_train = X_trainsets[train_index]
        y_train = y_trainsets[train_index]
        X_test = X_trainsets[test_index]
        y_test = y_trainsets[test_index]
        i = 0
    ...
    for (model_name, clf) in estimators:
        i += 1
        clf.fit(X_trainsets, y_trainsets) # Rerain the models on the whole dataset
    ...
    for r in range(ran):
        ...
        for n_estimators in parameters['n_estimators']:
            for max_depth in parameters['max_depth']:
                for max_features in parameters['max_features']: 
                    for learning_rate in parameters['learning_rate']: 
                        clf = GradientBoostingClassifier(random_state=r, subsample=1.0)
                        estimators.append(("GB", clf))    
        ...
        os.makedirs(f"modeltest/sample{fc}{r}", exist_ok=True) # change number 1: Mor; 2: Sta; 3: Mor&Sta features    
        i = 0
        for (model_name, clf) in estimators:
            i += 1
            clf.fit(X, y) # Rerain the models on the whole dataset
            with open(f"modeltest/sample{fc}{r}/base_model_{i}.pkl", "wb") as f: # change number
                pickle.dump(clf, f)
        print("\nBase models saved.")
        blr.append(best_index_LR)
        brf.append(best_index_RF)
        bgb.append(best_index_GB)
    return blr, brf, bgb
